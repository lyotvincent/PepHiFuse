...
from terminator.trainer import CustomTrainer, get_trainer_dict, SelfDefined_Model, SelfDefined_Config
...
class DataTrainingArguments:
    ...
    eval_data_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "Input evaluation data file to evaluate the delta_rt on (a text file)."
        },
    )
...
def main():
    ...
    selfdefined_config = SelfDefined_Config(config.to_dict(), 3, 2)
    if model_args.model_name_or_path:
        ...
        model = SelfDefined_Model.from_pretrained(
            model_args.model_name_or_path,
            from_tf=bool(".ckpt" in model_args.model_name_or_path),
            config=selfdefined_config,
            cache_dir=model_args.cache_dir,
            tokenizer=tokenizer,
        )
        ...
        model = SelfDefined_Model(selfdefined_config,tokenizer)
    ...
    for i in range(len(train_dataset)):
        train_dataset.examples[i] = tokenizer.add_padding_tokens(train_dataset.examples[i], data_args.block_size)
    for i in range(len(eval_dataset)):
        eval_dataset.examples[i] = tokenizer.add_padding_tokens(eval_dataset.examples[i], data_args.block_size)
    if training_args.do_eval:
    ...
    if selfdefined_config.transformer_config["model_type"] != "xlnet":
        warnings.warn(f"Full functionality only with XLNet; not {selfdefined_config.transformer_config['model_type']}")
    ...
    #     # performance of the model after the final training step.
