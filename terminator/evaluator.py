...
class Evaluator(CustomTrainer):
    ...
    def property_prediction(self, collator, save_path=None, rmse_factor: int = 1):
        ...
        # # Hack for XLNet tokenizer, 'decode_internal' is not achieved in ExpressionBertTokenizer.
        ...
        for prop in collator.property_tokens:
            ...
            logits, label_ids, metrics, input_ids = self.prediction_loop(
                dataloader=eval_dataloader,
                description=f"Predicting {prop}, the prediction shoule be only on pp task.",
                prediction_loss_only=False,
                return_inputs=True,
                pad_idx=self.tokenizer.vocab["[PAD]"],
            )
            ...
            # Compute performance (only on the logits where predictions are relevavnt)
            print("Obtained logits, now performing greedy and sampling search...")
            greedy_preds = self.greedy_search(relevant_logits).unsqueeze(0)
            sampling_preds = self.sampling_search(relevant_logits).unsqueeze(0)
            # beam_preds, scores = self.beam_search(relevant_logits)
            # beam_preds = beam_preds.permute(2, 0, 1)
            ...
            # Define rmse function
            rmse = lambda x, y: np.sqrt(sum((np.array(x) - np.array(y)) ** 2) / len(x))
            abs_rmse = lambda x, y: np.absolute((np.array(x) - np.array(y)))
            ...
            absrmses = np.zeros([len(all_preds), len(relevant_logits)])
            for idx, predictions in enumerate(all_preds):
                ...
                for sidx, (x, y, yhat) in enumerate(
                    ...
                    if random.random() < 0.00002:
                        print(f'Evaluation: target_prop is {target_prop}, gen_prop is {gen_prop}')
                ...
                ar = abs_rmse(property_labels, property_predictions[:, idx])
                if idx == 0:
                    print("GREEDY Preds: ")
                    print(f"Pearson is {p[0]}")
                    print(f"RMSE is {r}")
                    print(f"MAE is {np.median(ar)}")
                    print(f"Spearman is {s[0]}")
                # else:
                #     print("SAMPLING Preds, not to use")
                #     print(f"Pearson is {p[0]}")
                #     print(f"RMSE is {r}")
                #     print(f"MAE is {np.median(ar)}")
                #     print(f"Spearman is {s[0]}")
                pearsons[idx] = p[0]
                rmses[idx] = r
                assert len(ar) == len(relevant_logits)
                absrmses[idx] = ar
                spearmans[idx] = s[0]
            if save_path is not None:
                bw = self.beam_search.beam_width
                ...
                pd.DataFrame(
                    ...
                    os.path.join(save_path, "Evaluation(test).csv")
                )

        # self.tokenizer.decode = self.tokenizer.real_decoder

        return pearsons, rmses, spearmans, metrics, absrmses
