...
from transformers import Trainer, XLNetTokenizer, AutoModelWithLMHead, PretrainedConfig, PreTrainedModel, CONFIG_MAPPING, T5Tokenizer, AutoModel
...
from terminator.search import SEARCH_FACTORY, GreedySearch
...
class CustomTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        # performance evaluation for pearson, rmse, spearman
        self.perfs = np.tile(
            np.expand_dims(np.vstack([10 * np.arange(4)] * len(tokens)), -1),
            10000,
        ).astype(float)
        self.perfs[:, 1, :] = 1000  # RMSE
        self.perfs[:, 0, :] = 0  # Pearson
        self.perfs[:, 2, :] = 0  # Spearman
        self.perfs[:, 3, :] = 1000  # MAE
        self.cidx = 0
        ...
        # save_attention_func has 'self.counter' bug (not defined).
    ...
    def save_attention_func(self, inputs: torch.Tensor, attention: torch.Tensor):
        """
    ...
    def feed_model(
        ...
        # NOTE: Overwritten for monitoring purposes (will print occassionally)
        random.seed(time())
        if self.verbose_evaluation and random.random() < 0.001:
            logger.info(f"\nCheck if tokens between property and separator (or after the last separator in cg mode) are Mask (others should not be Mask),\nand the length equals to block_size.\n{self.tokenizer.convert_ids_to_tokens(inputs['input_ids'][0])}")
        ...
        if self.use_numerical_encodings:
            ...
            assert embeddings.shape[0] == inputs["input_ids"].shape[0] and embeddings.shape[1] == inputs["input_ids"].shape[1]
            ...
            if not self.save_attention:
                ...
                self.save_attention_func(inputs["input_ids"], outputs[-1])
        else:
            ...
            [model_inputs.pop(k, None) for k in NON_MODEL_KEYS]
            temp_inputs = []
            for temp_index in range(model_inputs["input_ids"].size(0)):
                temp_inputs.append(self.tokenizer.convert_ids_to_tokens(model_inputs["input_ids"][temp_index]))
            outputs = model(dot_index = self.tokenizer.vocab["_._"], decoded_input = temp_inputs, **model_inputs)

        if inputs.get("labels") is None:
            return (outputs[0],)

        # passing new weight to cross-entropy loss for property prediction
        logits = outputs[0]
        rt_weight = torch.ones(logits.size(-1), dtype=torch.float).to(device='cuda')
        for num_temp in range(10):
            rt_weight[self.tokenizer.vocab["_-_"]] = 1
            rt_weight[self.tokenizer.vocab["_+_"]] = 1
            rt_weight[self.tokenizer.vocab[f"_{num_temp}_2_"]] = 2
            rt_weight[self.tokenizer.vocab[f"_{num_temp}_1_"]] = 2
            rt_weight[self.tokenizer.vocab[f"_{num_temp}_0_"]] = 2
            rt_weight[self.tokenizer.vocab[f"_{num_temp}_-1_"]] = 1
            rt_weight[self.tokenizer.vocab[f"_{num_temp}_-2_"]] = 0.1
        loss_fct = CrossEntropyLoss(weight=rt_weight, reduction='mean')
        ...
        #     greedy_search_temp = GreedySearch()
        #     print(greedy_search_temp(outputs[0][:4,:11,:]))
        #     print(inputs["labels"][:4,:11])

        # Overwrite the loss with the new one
    ...
    def prediction_step(
        ...
        if self.verbose_evaluation and random.random() < 0.001:
            ...
            try:
                prediction = (
                    self.search(logits[0, :, :].unsqueeze(0))
                    .detach()
                    .cpu()
                    .squeeze()
                    .tolist()
                )
                for lid, lbool in enumerate(labels[0].eq(-100)):
                    if lbool:
                        prediction[lid] = inputs["input_ids"][0][lid].item()
                ...
                logger.info(f"Predicted: {p_seq} \t, {p_dict.get('rt', -1)}")
                logger.info(f"Ground truth {gt_seq} \t {gt_dict.get('rt', -1)}")
            except Exception:
    ...
    def prediction_loop(
        ...
        eval_losses_gatherer = DistributedTensorGatherer(world_size, num_examples, padding_index=pad_idx)
        preds_gatherer = DistributedTensorGatherer(world_size, num_examples, padding_index=pad_idx)
        labels_gatherer = DistributedTensorGatherer(world_size, num_examples, padding_index=pad_idx)
        inputs_gatherer = DistributedTensorGatherer(world_size, num_examples, padding_index=pad_idx)
        ...
        if self.alt_training:
            logger.warning(f"Running prediction on pp task and cg task every alternate steps.")
        else:
            logger.warning(f"Running prediction only on pp task.")
    ...
    def train(
            ...
            for step, (inputs, a_inputs) in enumerate(
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (
                        ...
                        metrics = self.property_evaluate()
        ...
        perfs_save_path = os.path.join(self.args.output_dir, "performance.npy")
        np.save(perfs_save_path,self.perfs)
    ...
    def property_evaluate(self):
        ...
        for pidx, prop in enumerate(properties):
            ...
            ps, rs, ss, metrics, ars = evaluator.property_prediction(
            ...
            if rs[0] < np.min(self.perfs[pidx, 1, :]):
                ...
                with open(save_path, "w") as f:
                    json.dump({"rmse": rs[0], "mae": np.median(ars[0])}, f)
                logger.info(f"New best RMSE: {rs[0]}")
            
            if np.median(ars[0]) < np.min(self.perfs[pidx, 3, :]):
                # Save model
                checkpoint_folder = (
                    f"{PREFIX_CHECKPOINT_DIR}-mae-min-{self.global_step}"
                )
                output_dir = os.path.join(self.args.output_dir, checkpoint_folder)
                self.save_model(output_dir)
                # Save optimizer and scheduler
                ...
                checkpoint_prefix = f"{PREFIX_CHECKPOINT_DIR}-mae-min"
                if self.is_world_process_zero():
                    self._rotate_checkpoints(prefix=checkpoint_prefix)

                save_path = os.path.join(output_dir, f"best_mae_{prop[1:-1]}.json")
                with open(save_path, "w") as f:
                    json.dump({"rmse": rs[0], "mae": np.median(ars[0])}, f)
                logger.info(f"New best MAE: {np.median(ars[0])}")

            # if ps[0] > np.max(self.perfs[pidx, 0, :]):
            #     # possible unexpected output (nan) which leads to not save the model in the following steps
            #     # Save model
            #     checkpoint_folder = (
            ...
            self.perfs[pidx, 3, self.cidx] = np.median(ars[0])

        # logger.info(f"Current prediction performances {self.perfs[:,:,:self.cidx+1]}")
        self.cidx += 1

        # only return the metrics for the last property
        return metrics
...
class SelfDefined_Config(PretrainedConfig):
    model_type = "selfdefined"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.minute_digits = minute_digits
        self.second_digits = second_digits
        super().__init__(**kwargs)

class SelfDefined_Model(PreTrainedModel):
    config_class = SelfDefined_Config
    base_model_prefix = "SelfDefined"

    def __init__(self, config, tokenizer):
        super().__init__(config)

        self.expression_tokenizer = tokenizer
        self.tokenizer = T5Tokenizer.from_pretrained('prot_t5_xl_uniref50', do_lower_case=False)
        self.embedding_model = T5EncoderModel.from_pretrained("prot_t5_xl_uniref50").to(device='cuda')
        for param in self.embedding_model.parameters():
            param.requires_grad = False

        from torch_geometric.nn import GCNConv
        
        self.gnn_conv1 = GCNConv(1024, 768)
        self.gnn_conv2 = GCNConv(768, 512)

        transformer_config_class = CONFIG_MAPPING[config.transformer_config["model_type"]]
        transformer_config = transformer_config_class.from_dict(config.transformer_config)
        self.model_prediction = AutoModel.from_config(transformer_config).to(device='cuda')

        self.weight_linear = nn.Linear(512, 1)
        
        self.fc = nn.Sequential(
            nn.Linear(1024, 512),
            nn.ReLU(),
            nn.Linear(512, config.transformer_config["vocab_size"])
        )

    def forward(self, dot_index, decoded_input, **input_kwargs):
        for i in range(len(decoded_input)):
            decoded_input[i] = ['</s>' if x == '[SEP]' else '<pad>' if x == '[PAD]' else x for x in decoded_input[i]]
        decoded_input = [' '.join(decoded_input[i][10:]) for i in range(len(decoded_input))]
        
        ids = self.tokenizer(decoded_input, add_special_tokens=True, padding="longest")

        seq_len = []
        for i in range(len(ids['input_ids'])):
            ids['input_ids'][i] = ids['input_ids'][i][:-1]
            ids['attention_mask'][i] = ids['attention_mask'][i][:-1]
            if 2 in ids['input_ids'][i]:
                assert decoded_input[i][ids['input_ids'][i].index(2)*2] == '1'
            # assert no '▁X' in the input
            assert 23 not in ids['input_ids'][i]
            eos_index = ids['input_ids'][i].index(self.tokenizer.get_vocab()['</s>'])
            ids['input_ids'][i][eos_index+1:] = [0] * (len(ids['input_ids'][i]) - eos_index - 1)
            ids['attention_mask'][i][eos_index+1:] = [0] * (len(ids['attention_mask'][i]) - eos_index - 1)
            seq_len.append(eos_index)
        
        t5_input_ids = torch.tensor(ids['input_ids']).to(device='cuda')
        t5_attention_mask = torch.tensor(ids['attention_mask']).to(device='cuda')

        # with torch.no_grad():
        embedding_repr = self.embedding_model.encoder(input_ids=t5_input_ids,
        ...
        # GNN model
        all_edge_index = []
        for b in range(embedding_repr.last_hidden_state.shape[0]):
            row, col = [], []
            for i in range(seq_len[b]):
                for j in range(max(0, i - 3), min(seq_len[b], i + 3 + 1)): # set k = 3
                    if i != j:
                        row.append(i)
                        col.append(j)
            edge_index_single = torch.tensor([row, col], device=embedding_repr.last_hidden_state.device, dtype=torch.long)  # [2, num_edges]

            offset = b * 118
            all_edge_index.append(edge_index_single + offset)
        batch_edge_index = torch.cat(all_edge_index, dim=1)  # shape: [2, batch_size * num_edges]

        x = embedding_repr.last_hidden_state.view(-1, 1024)
        x = F.relu(self.gnn_conv1(x, batch_edge_index))
        x = F.relu(self.gnn_conv2(x, batch_edge_index))
        x = x.view(embedding_repr.last_hidden_state.shape[0], 118, 512)

        pooled = []
        for b in range(embedding_repr.last_hidden_state.shape[0]):
            if seq_len[b] > 0:
                valid_feature = x[b, :seq_len[b], :]  # shape: [valid_len, hidden_size]
                pooled_feature = valid_feature.mean(dim=0)  # shape: [hidden_size]
            else:
                pooled_feature = x[b, 0, :]
            pooled_feature_expanded = pooled_feature.unsqueeze(0).expand(128, -1)  # shape: [seq_len, hidden_size]
            pooled.append(pooled_feature_expanded)
        pooled = torch.stack(pooled, dim=0)  # shape: [batch_size, seq_len, hidden_size]

        input_kwargs.pop("labels", None)
        xlnet_embeddings = self.model_prediction(**input_kwargs)[0]

        weights = self.weight_linear(pooled).squeeze(-1)
        weights = torch.sigmoid(weights)
        pooled = pooled * weights.unsqueeze(-1)
        
        x = torch.cat((pooled, xlnet_embeddings), dim=-1)
        logits = self.fc(x)

        # monitoring
        if random.random() < 0.0001:
            assert t5_input_ids.shape[1] == 128 - 10 and t5_attention_mask.shape[1] == 128 - 10
            assert embedding_repr.last_hidden_state.shape[1] == 128 - 10
            assert x.shape[1] == 128 and x.shape[2] == 1024
            assert logits.shape[1] == 128 and logits.shape[-1] == 306
            print(input_kwargs["input_ids"][:4,:11])
            print(input_kwargs["perm_mask"][:4,:11,:11])
            print(input_kwargs["attention_mask"][:4,:11])
            print(input_kwargs["target_mapping"][:4,:11,:11])

        return (logits,)
    
    def transformer_resize_token_embeddings(self, tokenizer_length):
        self.model_prediction.resize_token_embeddings(tokenizer_length)

    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        model = cls(kwargs["config"], kwargs["tokenizer"])
