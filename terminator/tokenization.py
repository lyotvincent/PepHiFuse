...
class ExpressionBertTokenizer(BertTokenizer):
    ...
    def get_sample_label(mlm_label: List[str], mlm_input: List[str]) -> List[str]:
        ...
        assert len(mlm_label) == len(mlm_input)
        for el, i in zip(mlm_label, mlm_input):
            if i == "[MASK]":
                assert el not in ["[UNK]", "[PAD]"]
    ...
    def get_sample_prediction(
        ...
        assert len(mlm_prediction) == len(mlm_input)
    ...
    def floating_tokens_to_float(token_ids: List[str]) -> float:
        ...
        try:
            ...
            temp = list(float_string)
            for i in range(len(temp)):
                if i > 0 and temp[i] not in ".0123456789":
                    temp[i] = "0"
            float_string = "".join(temp)
    ...
    def aggregate_tokens(
        ...
        if random.random() < 0.00001:
            print(f'Check if tokenization works well,\nbefore: {token_ids[:token_ids.index("[PAD]")]}\nafter: {property_dict}')
