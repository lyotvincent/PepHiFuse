...
class BaseCollator(DataCollatorForPermutationLanguageModeling):
    ...
    def mask(
        ...
        # Mask indicating non-functional tokens, where functional tokens are [SEP], [CLS] (2 special_tokens), padding (1 padding_token), etc.
    ...
    def mask_tokens(
        ...
        for search_idx in [self.property_token_idx, self.separator_token_idx]:
            ...
            if list(range(inputs.size(0))) != sample_matches.tolist():
                logger.error('''need to fix bugs in match_positions: 
                             the length for self.property_token_idx is not equal to that for self.separator_token_idx''')
                import sys
                sys.exit()
        ...
        if self.mask_token_order is None:
            ...
            for i in range(masked_indices.shape[0]):
                ...
                # make sure that the masked tokens are behind the separator
                assert torch.all(mask_tokens < matches[1][i])
